...
## ☁️ เปิดซิงค์คลาวด์ (ใช้หลายเครื่อง ข้อมูลไม่หาย) — ไม่บังคับ

ปกติแอพเก็บข้อมูลในเครื่อง (localStorage) ถ้าอยากให้ **ซิงค์ข้ามเครื่อง + ไม่หายแม้ล้างเบราว์เซอร์** ต่อ Upstash Redis (ฟรี):

1. สมัคร https://upstash.com (เข้าด้วย GitHub/Google ได้)
2. **Create Database** → เลือก Redis → ตั้งชื่อ → Region ใกล้ไทย (Singapore) → Create
3. ในหน้า DB เลื่อนหา **REST API** → คัดลอก 2 ค่า:
   - `UPSTASH_REDIS_REST_URL`
   - `UPSTASH_REDIS_REST_TOKEN`
4. ไป Render → service `oommoney` → **Environment** → เพิ่ม 2 ตัวนี้ → **Save Changes** (deploy ใหม่เอง)
5. เปิดแอพ → ตั้งค่า → จะเห็น **"ซิงค์คลาวด์: เปิดอยู่ · ข้อมูลซิงค์ทุกเครื่อง ☁️"**

> ครั้งแรกที่เปิด ข้อมูลในเครื่องจะถูกอัปขึ้นคลาวด์ให้เอง · เครื่องอื่นเปิดมาก็เห็นข้อมูลเดียวกัน (มีรหัสผ่านป้องกันเหมือนเดิม)
> ถ้าไม่ใส่ env 2 ตัวนี้ = ใช้แบบเดิม (เก็บในเครื่อง) ไม่มีอะไรเปลี่ยน
